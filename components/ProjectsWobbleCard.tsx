...
function ProjectsWobbleCard({
  ...
  backgroundColor,
  imageposition,
}: ProjectsWobbleCardProps) {
  return (
    <Link href={url} data-cuelume-hover="sparkle" className={cn(``, gridSpan)}>
      <WobbleCard
        containerClassName={cn(
          `h-full min-h-[400px] lg:min-h-[300px] group`,
          backgroundColor
        )}
        className=""
      >
        ...
        <Image
          ...
          alt={title}
          className={cn(
            `absolute  -z-40 grayscale group-hover:grayscale-0 filter object-contain rounded-2xl transition-all duration-200 ease-in-out`,
            imageposition
          )}
        />
      </WobbleCard>
