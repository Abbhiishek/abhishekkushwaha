    ...
    <div className="w-full h-full p-10 grid grid-cols-1 md:grid-cols-3 md:grid-rows-3  max-w-7xl mx-auto gap-4 relative">
      {cards.map((card, i) => (
        <div key={i} className={cn(card.className, 'h-60')}>
          <motion.button
            type="button"
            aria-label={selected?.id === card.id ? 'Close image' : 'Open image'}
            onClick={() => handleClick(card)}
            data-cuelume-hover="sparkle"
            data-cuelume-toggle={selected?.id === card.id ? 'droplet' : 'bloom'}
            className={cn(
              card.className,
  ...
  return (
    <div className="bg-transparent h-full w-full flex flex-col justify-end rounded-lg shadow-2xl relative z-[60]">
      ...
      <motion.div
        ...
        className="relative px-8 pb-4 z-[70] text-white font-aclonica"
      >
        {selected?.content}
      </motion.div>
    </div>
